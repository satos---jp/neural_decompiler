...
The foregoing is the basic structure of the sequence-to-sequence model.
To improve the performance of the model, there are some devices for the model.
...
\begin{figure}
	\begin{tabular}{|c|c|c|c|}
		\hline
		 model & Seq2seq & Seq2seqAtt & Seq2tree \\ \hline
		 Predict & \multicolumn{2}{|c|}{next token} & next action \\ \hline
		...
		% Vector Dim & \multicolumn{3}{|c|}{128} \\ \hline
	\end{tabular}
...
Figure~\ref{fig:editdist} shows the relation of trainig data size and the edit disatnce for each models.
Figure~\ref{fig:bleu} shows the relation of trainig data size and the BLEU score for each models.
...
\chapter{Discussion}
結果より、まずattention機構によって有意に精度が向上することが確認できた。
しかしながら、 seq2seq と seq2tree の間に
