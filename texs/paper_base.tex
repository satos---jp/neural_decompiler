...
Second is the multi-layer reccurent neural networks, introduced in ここ決める.
...
Attention is a extension module of sequence-to-sequence model to improve the quality of translation. 
...
We implemented three models, Seq2seq, Seq2seqWithAattention, and Seq2tree.

The encoding module was bidirectional LSTM in all of three models.
The decoding module of Seq2seq and Seq2seqWithAattention predict the conditional probability of a next token $ p(sy_k|sx,[sy_1,\dots,sy_{k-1}]) $.
The decoding module of Seq2tree predicts the conditional probability of a next action $ p(a_{k}|sx,[a_1, \dots a_{k-1}]) $.

Seq2seqWithAattention and Seq2tree have attention module, while Seq2seq have not.

\begin{figure}
	\begin{tabular}{c|c}
		Number of Layers & 4
		%Dimensionality of Embedded Vectors & 128
	\end{tabular}
	\caption{The configuration parameter of the models}
	\label{fig:parameterofmodels}
\end{figure}

Figure~\ref{fig:parameterofmodels} shows the parameter of the models. 
...
BLEU is the metrics for the naturallity of the generated program. 
